...
**And the same day, 王弼 was found to be complete, and the worklist caught up with itself.**

- **王弼 (*Wáng Bì*) now covers all 81 chapters, not 71.** The ten recorded for two years as *"unproofread"* (8, 14, 15, 19, 30, 54, 62, 70, 71, 78) were in the transcription all along. Their headings ran onto the end of the previous chapter's last comment line, and the importer merged each into its neighbour. Recovering them surfaced new forks: at ch 15, 若容 (*ruò róng* — like a bearing) where we print 若客 (*ruò kè* — like a guest), and 久 (*jiǔ* — long) now attested by both commentaries. The comments on all ten have never been read against decisions made without them. That read is [`WORKLIST.md`](WORKLIST.md) R3.
- **說文 (*Shuōwén*) now covers 763 of the book's 798 characters, up from 711.** 52 characters are now linked to the older graph 說文 files them under, each checked against the vendored text. Four are refused on purpose, where our character is only a borrowed graph and the entry defines a different word.
- **`--formulas` finds repeats that sit inside a longer segment.** Within-chapter findings rose from 152 to 181, and ch 39's 為天下貞 (*wéi tiān xià zhēn* — "serves as the world's standard") joined its frame.
- **`repeated-formula` stopped crying wolf.** All seven of its warnings were two identical renderings, one sharing its verse line with the next clause. The check now measures containment, and the gate reads 0 errors and 2 warnings, both of them calls owed to Shalom.
- **Two of Shalom's calls, applied.** Ch 38's ladder now follows 韓非 (*Hán Fēi*)'s cascade, *"When the Tao is lost, then integrity is lost,"* departing from the base, because his own essay argues each rung is made from the one above. Ch 14's 古始 (*gǔ shǐ*) is *the ancient beginning*, no longer *the primordial source*. Ch 47 restores the received 名 (*míng* — to name), *"names without seeing"*, after the silk's 明 (*míng* — clear-seeing) had sat in our Chinese since the Google Doc. And ch 65's *"make the people clever"* is now recorded as 明's one declared exception, following 王弼 (*Wáng Bì*): in that line 明 is done *to* a people, and the lock's English would have the sages keeping people from seeing clearly.
- **The ch 2 and ch 26 apparatus entries were written**, and ch 26's had 主 (*zhǔ* — lord) and 王 (*wáng* — king) the wrong way round. R1, the 韓非 (*Hán Fēi*) harvest, had been done since 2026-09-10 and never checked off. The chapter-status section, which still listed Pass D's chapters as owed, was retired.
