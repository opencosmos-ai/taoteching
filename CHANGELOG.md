...
- **And the project's three skills had not been loadable at all** since the repository moved from `shalomormsby/taoteching` to `opencosmos-ai/taoteching`: they are exposed by absolute symlinks in `~/.claude/skills`, and all three were dangling. Shalom noticed `principle-entry` was missing. Relinked; `process/skills/README.md` has the loop.
