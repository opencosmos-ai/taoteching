...
| T2-56 | ⬜ | ★ **治 (*zhì*) wants an entry — 12 lines, 9 chapters, and the lock 亂 rests on** — *govern* at 3 ×2, 10, 57, 59, 60, 65 ×3, 75 ×2; ***order*** at 8 and 64. **The outlier is 政善治 (8)**, where 政 (*zhèng* — governing) and 治 stand in one three-character line so the English needs two words — **that single line may be what forces the second rendering at all**. Until it is settled, `luan-亂.md`'s 治/亂 pairing is held by ch 64 alone. **Third character in the room:** *order* is also on 正 at ch 45, which is **deferred** (T4-2), so 治 must be taken without settling 正 | 3 8 10 57 59 60 64 65 75 | E |
