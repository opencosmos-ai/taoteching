...
| T2-54 | ⬜ | **昧 (*mèi*) reads *dark*, which is 玄's word** — 明道若昧 (41) and 其下不昧 (14), and `--english "dark"` shows both unbacked because there is no 玄 in either chapter. 玄 & 妙 are locked to *dark or profound / subtle*. **`ming-明.md` glosses 昧 as *dim* in its own argument**, so our entry and our verse disagree. 說文 on 昧 is **爽，旦明也…一曰闇也** — *"dawn brightness… one source says 闇 (àn — dark),"* so the character genuinely spans both, and the case is **not** that *dark* misreads 昧 but that *dark* is **already spoken for**. Ch 20's 我獨昏 (*hūn*, 說文 日冥也 — the sun gone down) reads *dark* too and travels with it: three characters, one English | 14 20 41 | E |
