...
| T2-56 | 🔶 | **治 (*zhì*) — entry written 2026-09-23, `status: open`.** **說文 files it as a river**: 治，水。出東萊曲城陽丘山，南入海 — the governing sense is the extension, and the metaphor is 大禹治水, channelling rather than damming. 王弼 at 60: 治大國若烹小鮮 → **不擾也**, *not to disturb*. **Recommended *govern*** (10 of 12) **with a declared flexion to *order* at 8 and 64**, where the object is not a polity. **Owed before it locks: 政** (*zhèng* — government), which reads *In governing* at 8 and *the government* at 58 ×2. 正 (*zhèng* — upright) is adjacent and **deferred** (T4-2), so 政 must be settled without it | 3 8 10 57 58 59 60 64 65 75 | E |
