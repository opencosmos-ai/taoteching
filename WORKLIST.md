...
| T2-54 | 🔶 | **昧 → *murky* at 14 and 41** (Shalom's call, 2026-09-20), because ***dark* is 玄's** — six verse lines including ch 1's 玄之又玄, and **no 玄 in either chapter**. 說文 gives 昧 both senses (昧爽，旦明也…一曰闇也), so the fault was the collision, not a misreading. *Dim* is 和 and 冥, *faint* is 微, *dull* is 爽 and 悶; the chime with 濁 (*muddy*) was **weighed and accepted as resonance**. **Still open: 昏** (*hūn*, 說文 日冥也) reads *dark* at ch 20's 我獨昏, so 玄's word is still worn twice. **And 昧 is settled but unlocked**, so the atlas publishes the pre-lock gloss — the T2-40 debt again | 14 20 41 | E |
